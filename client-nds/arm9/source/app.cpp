...
void app::tick() {
    ...
    uint16_t new_keys = keysCurrent();
    if (new_keys != last_keys && connected) {
        send_data();
        last_keys = new_keys;
    }
}
...
void app::loop() {
    while (1) {
        ...
        consoleClear();
        u8 addr_bytes[8];
        ip_u32_to_u8_arr(client_ip, addr_bytes);
